...
momentum = float(sys.argv[6])#0.9
wd = float(sys.argv[7])#1e-4
log_interval = 10
...
def get_data():
    train_set = torchvision.datasets.FashionMNIST('./../../files/', train=True, download=False,
                            transform=torchvision.transforms.Compose([
                            torchvision.transforms.ToTensor()
    ]))

    noisy_prob = float(sys.argv[1])
    noisy_indices = list(range(train_set.targets.size()[0])) #new_train_set.copy()
    ...
    train_subset_loader  = torch.utils.data.DataLoader(train_set, batch_size=batch_size_train,drop_last=True)

    test_set = torchvision.datasets.FashionMNIST('./../../files/', train=False, download=False,
                                transform=torchvision.transforms.Compose([
                                torchvision.transforms.ToTensor()
                                ]))
