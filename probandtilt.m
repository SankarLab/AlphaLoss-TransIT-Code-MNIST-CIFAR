...
for i = 1:1:k
    for j = 1:1:M
        if a(i) == -4
            f(j,i) = (1-nchoosek(20,y(j))*(p)^(y(j))*(1-p)^(20-y(j)))^(-a(i));
        else
            f(j,i) = (nchoosek(20,y(j))*(p)^(y(j))*(1-p)^(20-y(j)))^(a(i));
        end
    end
    b(i) = sum(f(:,i));
    for j = 1:1:M
...
for j = 1:1:n
    for i = 1:1:M
        if a(j) == -4
            f(i,j) = (-a(j)/(-a(j)-1)*(1-(1-x(i))^(1-1/(-a(j)))));
...
lgd = legend({'\alpha = .35','\alpha=.5','\alpha = .7','\alpha = 1 [Log-Loss]', '\alpha = 1.8','\alpha = \infty [0-1 Loss]'},'FontSize',14,'Location','northeast');
lgd.FontSize = rad1;
title('(a) \alpha-loss','FontSize',rad)
ylim([0, 5])
set(gca,'fontsize',16)
